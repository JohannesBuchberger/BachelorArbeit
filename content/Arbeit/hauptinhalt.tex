...
Neben der User Task gibt es die Manual Task, die mit einem Handsymbol gekennzeichnet ist. Eine manuelle Aufgabe bedeutet, dass etwas erledigt werden muss, das keine Verbindung zur Workflow-Engine oder sonstige technische Eingaben benötigt. \citep[vgl.][S. 34]{bruce_englisch_2011}
...
Ebenfalls wird dieser Stelle nochmal an \fullref{par:spezifikation_von_prozessgesteuerten_anwendungen} verwiesen, hier wurde das nachfolgende Vorgehen bereits grundlegend erklärt und wird nun für jeden der folgenden Prozesse, sofern sich daraus erarbeitete Informationen nicht doppeln, angewendet.
...
Wie in \fullref{par:spezifikation_von_prozessgesteuerten_anwendungen} ausgeführt, werden nun die Nachrichtenflüsse zu diesem Prozess an dieser Stelle beschrieben. Um eine gewisse Schnittstelle zwischen den Prozessen zu dokumentieren, ist dies auch absolut notwendig. Auch damit diese unabhängig entwickelt werden können. 
...
Um nicht jedes Mal Anpassungen im Laufe der nächsten Prozessbeschreibungen an einem Datenmodell aufführen zu müssen, wird ein einheitliches Datenmodell mit der Aufschlüsselung der Geschäftsobjekte am Ende für alle Prozesse in \fullref{sec:datenmodell} zu finden sein.
...
\paragraphandnewline{Beteiligte Prozessrollen}
Die hier relevanten Rollen hierbei sind der speisende Kunde und das Restaurant.
...
\paragraphandnewline{Visualisierung des Prozessflusses}
\abbildung{0.8\textwidth}{img/proc;Selbstbedienungsprozess.png}{iref:Selbestbedienungsprozess}{Toplevel-Prozess für das Selbstbedienungsrestaurant}
...
Die aus dem Zahlungsprozess entstandene Rechnung existiert hauptsächlich als Nachweis, spielt aber gerade im Falle der ''Zahlungsrückabwicklung'' bei einer Stornierung eine wichtige funktionale Rolle. Die Rechnung kann dabei auch aus dem Bestellprozess entstehen, falls das Restaurant nur auch schon bezahlte Bestellungen entgegennehmen will.
...
\paragraphandnewline{Beteiligte Prozessrollen}
Die einzige hier existierende Prozessrolle ist der Kunde. Alle vom Restaurant zu erledigenden Schritte werden automatisch abgewickelt oder sind von einem externen Dienstleister zu erledigen.
...
\clearpage
\paragraphandnewline{Visualisierung Zahlungsvorbereitungsprozess}
...
\paragraphandnewline{Visualisierung Zahlungsvorbereitungsprozess}
\abbildung{0.9\textwidth}{img/proc;Zubereitungsprozess.png}{iref:Zubereitungsprozess}{Zubereitungsprozess}
...
\paragraphandnewline{Beschreibung des Datenflusses}
Die hier relevanten Informationen sind die Bestellung, und Kunde. Der Kunde wird wie immer im Modell nicht extra hervorgehoben. 
...
Sobald der Kunde das Essen abgeholt hat, endet der Prozess in dem Endereignis ''Abholung durchgeführt''. Sollte der Kunde allerdings das Essen nicht innerhalb von 15 Minuten abholen, muss sich ein Restaurantmitarbeiter manuell um das weitere Vorgehen kümmern. Daraufhin endet der Prozess ebenfalls.
...
\paragraphandnewline{Beteiligte Prozessrollen}
\clearpage
\paragraphandnewline{Visualisierung Zahlungsrückabwicklungsprozess}
